...
import { ChoroplethChart, ChoroplethFeatureComponent, ChoroplethGraticule, ChoroplethTooltip, PatternLines, PatternCircles } from "@bklitui/ui/charts";
import { ChoroplethDemo, ChoroplethAnalyticsDemo } from "@/components/docs/choropleth-demo";
import { ChoroplethLinesPatternDemo, ChoroplethDotsPatternDemo } from "@/components/docs/choropleth-pattern-demo";
...
Use `PatternCircles` for a dot pattern effect:
...
<ComponentShowcase code={`<ChoroplethChart data={geojson} aspectRatio="16 / 9">
  ...
  <ChoroplethFeatureComponent
    patterns={
      <>
        <PatternCircles id="dots-1" fill="var(--chart-1)" radius={2} ... />
        <PatternCircles id="dots-2" fill="var(--chart-2)" radius={2} ... />
      </>
    }
    getFeaturePattern={(feature) => getPatternId(feature)}
  />
